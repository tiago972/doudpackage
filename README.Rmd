...
variable, one column per sub-group, the right statistical test picked for you,
and an HTML or LaTeX table ready to paste into the manuscript.

You give it a `data.frame` and the name of the grouping variable. It works out
which variables are quantitative and which are qualitative, describes each of
them, compares them across the groups, and renders the result with
...
Two functions do the work: `descTab()` computes the table, `parseClassFun()`
renders it.
...
`descTab()` returns an S4 object; the raw data frame is under `["table"]` and
`parseClassFun()` turns it into the formatted table:
...
Variables of any other type (character, `Date`, logical) are ignored with a
warning. Set `normality` to choose how quantitative variables are handled:
...
* Unused levels of `group` (typically left behind by a subset) are dropped, with
  a warning, rather than producing an empty column.
* When a test cannot be computed — a constant variable, an empty level, too few
  observations — the p value is `NA` and a warning names the variable. The rest
